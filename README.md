...
它能够通过大语言模型自动通过检查您的文章，找出错别字、多字、少字等错误，并提供由用户审核的界面，确保修改的准确性。
...
- **🤖 模型兼容**: 支持 OpenAI 格式的 API，完美兼容 **Ollama（本地模型）**、OpenAI、DeepSeek 等。
...
```bash
pip install PySide6 openai tqdm markdown-it-py
```
...
**config.json 示例 (推荐使用本地模型，无隐私担忧):**

```json
{
  "OPENAI_API_KEY": "sk-xx",
  "OPENAI_API_BASE_URL": "http://localhost:11434/v1",
  "GPT_MODEL": "qwen3-8b-6:latest",
  "USER_PROMPT": "请认真校对以下句子。\n你只需要寻找错别字、多字、少字的错误。\n若原文存在错误，你回复的格式为：`[修改方法]`\n若整个句子没有问题，则回复“没有问题”/no_think",
  "REQUEST_DELAY_SECONDS": 2,
  "POSTS_DIR": "../source/_posts"
}
```

**注意：提示词：`若整个句子没有问题，则回复“没有问题”`，本项目使用关键词 `没有问题` 过滤无问题的句子，如果你要修改提示词，请保留这一句。**

**参数解释：**

- `OPENAI_API_BASE_URL`: API 地址。如果是本地 Ollama，通常是 `http://localhost:11434/v1`。
- `OPENAI_API_KEY`: API 密钥。本地模型通常随便填（如 `sk-xx`）。
- `GPT_MODEL`: 模型名称。
- `REQUEST_DELAY_SECONDS`: 两次请求之间的等待秒数（防止请求过快报错）。
...
**Q2: 如何使用 DeepSeek 或 Kimi 等在线 API？**

A: 修改 `config.json`：

- `OPENAI_API_BASE_URL`: 填对应厂商的 API 地址 (例如 `https://api.deepseek.com/v1`)。
- `OPENAI_API_KEY`: 填申请到的 密钥。
- `GPT_MODEL`: 填对应的模型名 (例如 `deepseek-chat`)。
